  ...
  Widget build(BuildContext context) {
    ...
    return Stack(children: [
      ...
      color: AppColors.kPrimaryColor,
        child: SafeArea(
          child: Column(children: [
            ...
            Spacer(),
            SideMenuItem(icon: "account",title: "Account Settings",top: 25,left:0.4*screenWidth),
            SideMenuItem(icon: "security",title: "Security Settings",top: 25,left:0.4*screenWidth),
            SideMenuItem(icon: "logout",title: "Logout",top: 25,left:0.4*screenWidth, color: AppColors.kRed,),
            YMargin(20)
          ],),
        ),
    ),
      AnimatedPositioned(
        ...
        left: -0.5*screenWidth,right: 0.67*screenWidth,
        child: Container(
      ...
      AnimatedPositioned(
          duration: Duration(milliseconds: 300),
          top: isCollapsed ?  0.1 * screenHeight:0,
          bottom:isCollapsed ? 0.1 * screenHeight:0,
          left:isCollapsed ? -0.5*screenWidth:0,right:isCollapsed ? 0.7*screenWidth:0,
          child: ScreenContainer(newSignUp: false,handleMoreClicked: (){
            setState(() {
              isCollapsed = !isCollapsed;
            });
          },)
...
class SideMenuItem extends StatelessWidget {
  ...
  final Color color;
  const SideMenuItem({
    Key key, this.title, this.icon, this.top = 0, this.left = 200, this.color = AppColors.kWhite,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        XMargin(left),
        SvgPicture.asset("images/$icon.svg", color: color,),
        XMargin(5),
        SizedBox(
          width: 140,
