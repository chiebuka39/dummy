...
class InvestmentService extends ABSInvestmentService {
  ...
  Future<Result<List<InvestmentMutualFund>>> getMutualFundValuation(
    ...
    var url = "$microService/api/Dashboards/mutualfundvaluation";
    print("url $url");
    try {
  ...
  Future<Result<List<InvestmentFixedFund>>> getFixedFundValuation(
    ...
    var url = "$microService/api/Dashboards/directfixedincomevaluation";
    print("url $url");
    try {
      var response = await dio.get(url, options: Options(headers: headers));
      final int statusCode = response.statusCode;
  ...
  Future<Result<List<InvestmentTermFund>>> getTermFundValuation(
    ...
    var url = "$microService/api/Dashboards/terminstrumentvaluation";
    print("url $url");
    try {
      var response = await dio.get(url, options: Options(headers: headers));
      final int statusCode = response.statusCode;
  ...
  Future<Result<List<MutualFund>>> getDollarFund({String token}) async {
    ...
    var url = "zimvest.services.investment/api/MutualFunds/getdollarfunds";
    print("url $url");
    try {
      var response = await dio.get(url, options: Options(headers: headers));
      final int statusCode = response.statusCode;
  ...
  Future<Result<dynamic>> buyMoneyMarketFund(
    ...
    var url = "$microService/api/MutualFunds/buymoneymarketfund";
    print("url $url");
  ...
  Future<dynamic> buyNairaInstrument(
      ...
      String uniqueName,
      String token}) async {
    var url = "$microService/api/ZimvestTermInstruments/buynairaterminstrument";
    print(url);
    var headers = {HttpHeaders.authorizationHeader: "Bearer $token"};
  ...
  Future buyDollarnstrumentWired(
    ...
    try {
      ...
      if (buydollaInstrumentWired.statusCode == 200) {
        ...
        return "Success";
      } else if (buydollaInstrumentWired.statusCode == 400) {
